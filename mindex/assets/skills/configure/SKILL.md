...
prepares by default, while plain `codex` stays available in its original
vanilla form unless the user explicitly asks to configure Mindex into that
Codex environment.
...
- keep plain `codex` unchanged by default, but if the user asks Codex to configure Mindex, apply the same managed instructions, packaged skills, and Mindex profile to that Codex environment
