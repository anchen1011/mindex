...
def default_pr_body(title: str) -> str:
    return "\n".join(
        [
            "## Summary",
            f"- {title}",
            "",
            "Generated by Mindex's automated branch and PR workflow.",
        ]
    )
...
def _ahead_count(project_root: Path, base_branch: str, *, env: dict[str, str] | None = None) -> int:
    compare_ref = f"origin/{base_branch}" if _git_ref_exists(project_root, f"origin/{base_branch}", env=env) else base_branch
...
def publish_pull_request(
    *,
    ...
    pr_body = body or default_pr_body(pr_title)
    ...
    try:
        ...
        branch = ensure_feature_branch(
            resolved_root,
            summary=branch_name or pr_title,
            branch_name=branch_name,
            env=env,
            log_run=log_run,
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--body", help="Pull request body; defaults to a generated summary")
