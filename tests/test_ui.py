...
class UiTests(unittest.TestCase):
    ...
    def test_reset_ui_config_rewrites_existing_settings(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
                ...
                json.dumps(
                    {
                        ...
                        "storage": {
                            "state_file": str(root / "old-state.json"),
                            "queue_log_dir": str(root / "old-logs"),
                        },
                        "ui": {"title": "Legacy UI"},
                    },
                    indent=2,
            ...
            bootstrap = ui_module.reset_ui_config(project_root=root, password="new-secret")
    ...
    def test_remote_ui_config_allows_local_interface_origins(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                mock.patch.object(ui_module.socket, "gethostname", return_value="deckbox"),
                mock.patch.object(ui_module.socket, "getfqdn", return_value="deckbox.local"),
                mock.patch.object(ui_module.socket, "getaddrinfo", side_effect=fake_getaddrinfo),
            ):
    ...
    def test_cli_routes_ui_init_config_prompts_for_password_when_missing(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir) / "repo"
            self._create_repo(root)
            stdout_buffer = io.StringIO()
            ...
            with (
                mock.patch.object(ui_module.getpass, "getpass", side_effect=["deck-secret", "deck-secret"]),
